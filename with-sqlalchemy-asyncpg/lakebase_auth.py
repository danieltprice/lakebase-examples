# Fetches short-lived DB tokens from Databricks; used for SQLAlchemy async (asyncpg) URL.
import base64
import json
import os
import threading
...
def _refresh() -> None:
    ...
    _expires_at = datetime.fromisoformat(
        data2["expire_time"].replace("Z", "+00:00")
    ).timestamp()
...
def get_connection_url() -> str:
    """Postgres URL for SQLAlchemy (postgresql+asyncpg)."""
    user = quote_plus(os.environ["DATABRICKS_CLIENT_ID"])
    password = quote_plus(get_password())
