# Feature flags with Databricks Lakebase and SvelteKit

This example shows feature flags backed by a Lakebase (Postgres) table, using SvelteKit.

## Clone the repository

```bash
npx degit databricks-solutions/lakebase-examples/with-sveltekit-feature-flags ./with-sveltekit-feature-flags
```
...
The app uses short-lived database tokens fetched automatically from Databricks — no manual credential rotation needed.
...
```bash
npm install
npm run db:setup   # seeds feature_flags table
npm run dev
```
