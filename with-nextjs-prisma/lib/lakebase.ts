...
async function getConnectionString(): Promise<string> {
  ...
  return `postgresql://${user}:${password}@${dbHost}:${port}/${database}?sslmode=require&connect_timeout=10&pool_timeout=10`;
}

const globalForPrisma = global as unknown as { prisma: PrismaClient | null };

/** Returns a PrismaClient with current Lakebase connection; cached and refreshed when token is stale. */
